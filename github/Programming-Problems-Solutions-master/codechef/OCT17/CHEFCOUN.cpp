...
int wrongSolver2(vector <unsigned int> a)
{
    int n = a.size();
    vector<unsigned long long> prefSum(n), sufSum(n);
    prefSum[0] = a[0];
    for (int i = 1; i < n; i++)
    {
        prefSum[i] = prefSum[i - 1] + a[i];
    ...
    unsigned long long mn = prefSum[0] + sufSum[0];
    int where = 1;
    for(int i = 1; i < n; i++)
    {
        unsigned long long val = prefSum[i] + sufSum[i];
        if (val < mn)
        {
            mn = val;
            where = i + 1;
...
vector<unsigned int> func(int n)
{
    ...
    {
        if(sum>=(1<<16)) res[i] += 1<<16, sum -= (1<<16);
        else
        {
            res[i] += sum;
            break;
...
//    ios_base::sync_with_stdio(0);
    ...
    while(ti--)
    {
        int n;
        scanf("%d", &n);
        vector<unsigned int> res = func(n);
//        if(res.size() != n) {
//            printf("eorrr");
//            continue;
//        }
