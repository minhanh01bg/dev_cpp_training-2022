...
//    freopen("00.txt", "r", stdin);
    ...
    for(int i = 2; i <= n; i++) {
        one.push_back(make_pair(i-1,i));
    }
    for(int i = 1; i <= k && i < n-1; i++) {
        two.push_back(make_pair(i,n));
    }
    if(k == n-1 && n>=2) two.push_back(make_pair(1,n-1));
    printf("%d\n", one.size()+two.size());
    for(int i = 0; i < one.size(); i++)
        printf("%d %d %d\n", one[i].first, one[i].second, 1+i);
    for(int i = 0; i < two.size(); i++)
        printf("%d %d %d\n", two[i].first, two[i].second, one.size()+1+i);
    return 0;
}
