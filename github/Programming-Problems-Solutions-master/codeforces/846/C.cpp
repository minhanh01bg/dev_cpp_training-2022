...
//    freopen("input.txt", "r", stdin);
    ...
    for(int i = 1; i <= n; i++)
    {
        prv[i] = make_pair(-mod,-1);
        for(ll j = 0; j <= i+1; j++)
        {
            ll total = sum(0,j)-sum(j,i+1);
            prv[i] = max(prv[i],make_pair(total,j));
        }
    }
    for(int i = 1; i <= n; i++)
    {
        nxt[i] = make_pair(-mod,-1);
        for(ll j = i; j <= n+1; j++)
        {
            ll total = sum(i,j)-sum(j,n+1);
            //deb(i,j,total);
            nxt[i] = max(nxt[i],make_pair(total,j));
        }
    }
    ll ans = -mod, a, b, c;
    for(int i = 0; i <= n; i++)
    {
        deb(prv[i].first, prv[i].second);
        //deb(nxt[i].first, nxt[i].second);
        if(ans<prv[i].first+nxt[i+1].first)
        {
            ans = prv[i].first+nxt[i+1].first;
            a = prv[i].second;
            b = i;
            c = nxt[i].second;
        }
