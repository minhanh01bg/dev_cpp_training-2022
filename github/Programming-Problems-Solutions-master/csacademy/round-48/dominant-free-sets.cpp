...
void update(int n, int b, int e, int i, ll val)
{
    if(i>e || b>i) return;
    if(b == e && i == b)
    {
    ...
    int m = (b+e)/2;
    update(n*2,b,m,i,val);
    update(n*2+1,m+1,e,i,val);
...
ll query(int n, int b, int e, int i, int j)
{
    ...
    int m = (b+e)/2;
    return (query(n*2,b,m,i,j)+query(n*2+1,m+1,e,i,j))%mod;
}
...
//    freopen("input.txt", "r", stdin);
    ...
    for(int i = n; i >= 0; i--)
    {
        ll val = query(1,1,MX,1,arr[i].second-1);
        update(1,1,MX,arr[i].second,val+1);
    }
    printf("%lld\n", tree[1]);
    return 0;
}
