...
int dx[]= {0,-1,-1};/*4 side move*/
int dy[]= {-1,0,-1};/*4 side move*/
...
//    freopen("input.txt", "r", stdin);
    ...
    while(q--)
    {
        ...
        for(int i = 0; i < n; i++)
            for(int j = 0; j < n; j++)
            {
                ...
                for(int k = 0; k < 3; k++)
                {
                    int tx = i+dx[k];
                    int ty = j+dy[k];
                    if(tx < 0 || ty < 0 || str[tx][ty] == 'X') continue;
                    cnt[grandy[tx][ty]] = 1;
                }
                int mx = 0;
                while(cnt[mx] == 1) mx++;
        ...
        for(int i = 0; i < n; i++)
            for(int j = 0; j < n; j++)
            {
                if(str[i][j] != 'K') continue;
                for(int k = 0; k < 3; k++)
                {
                    int tx = i+dx[k];
                    int ty = j+dy[k];
                    if(tx < 0 || ty < 0 || str[tx][ty] == 'X') continue;
                    int tem = ans^grandy[i][j]^grandy[tx][ty];
                    if(tem == 0) res++;
                }
            }
        printf("%d\n", res);
