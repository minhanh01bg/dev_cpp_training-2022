...
double a,b,c,d,e,f,x1,y11;

string check()
{
	double k = a * d + c * f + e * b - b * c - d * e - f * a;
	k = abs(k);
	k /= 2;
	if (k == 0) return "No solution";
	double AB = (a - c) * (a - c) + (b - d) * (b - d);
	AB = sqrt(AB);
	double AD = (a - x1) * (a - x1) + (b - y11) * (b - y11);
	AD = sqrt(AD);
//	cout<<AB / AD<<endl;
	if (AB / AD > k1) return "No solution";
//	cout<<AB / AD<<endl;
//	cout<<k1<<endl;
	double compare = k1 / (AB / AD);
	compare = 1 / compare;
//	cout<<compare<<endl;
	double tick1 = (1 - compare) * (a - e);
	double tick2 = (1 - compare) * (b - f);
	double x2, y2;
	x2 = e + tick1;
	y2 = f + tick2;
