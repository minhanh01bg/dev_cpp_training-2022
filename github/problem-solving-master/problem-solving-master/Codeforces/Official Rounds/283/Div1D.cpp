...
bool go(vector<Geo2D::Point> & a, vector<Geo2D::Point> & b, Geo2D::Point P, Geo2D::Point Q){
  double rad = P.distanceTo(Q);
  for(int i=0; i<b.size(); i++){
    Geo2D::Point c = b[i] + (P-Q);
    for(int j=0; j<a.size(); j++){
      Geo2D::Line l(a[j], a[(j+1)%a.size()]);
      if(Geo2D::distancePointSegment(c, l) <= rad && (a[j].distanceTo(c) >= rad || a[(j+1)%a.size()].distanceTo(c) >= rad)) return true;
    }
  }
  return false;
}
...
int32_t main(){
  ...
  int n;
  cin >> n;
  vector<Geo2D::Point> a(n);
  for(int i=0; i<n; i++) cin >> a[i].x >> a[i].y;
  cin >> Q.x >> Q.y;
  int m;
  cin >> m;
  vector<Geo2D::Point> b(m);
  for(int i=0; i<m; i++) cin >> b[i].x >> b[i].y;
  if(go(a,b, P, Q) ||  go(b,a, Q, P)) cout << "YES" << endl;
  else cout << "NO" << endl;  
}
