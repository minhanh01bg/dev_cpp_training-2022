...
{
    int b, w, u, p;
    data(int b, int w, int u, int p): b(b), w(w), u(u), p(p) {}
    data() {}
    bool operator < (const data & p) const
    {
        return abs(b-w)<abs(p.b-p.w);
    }
};
...
void dfs(int u, int p)
{
    ...
    if(u == 1) return;
    ans = max(ans,data(child_b[u], child_w[u], u, p));
    ans = max(ans,data(black-child_b[u], white-child_w[u], p, u));
}
