...
double getArea()
{
	...
	SORTC(C, compare);
	point p1 = C[0], p2 = C.back();
	vec(point) up, down;
	up.pb(p1);
	down.pb(p1);
	FOR(i,1,C.size()-1)
	{
		if (i == C.size() - 1 || clockwise(p1, C[i], p2))
		{
			while (up.size() >= 2 && !clockwise(up[up.size()-2], up[up.size() - 1], C[i])) up.pop_back();
			up.pb(C[i]);
		}
		if (i == C.size() - 1 || counterClockwise(p1, C[i], p2))
		{
			while (down.size() >= 2 && !counterClockwise(down[down.size()-2], down[down.size() - 1], C[i])) down.pop_back();
			down.pb(C[i]);
		}
	}
	C.clear();
	FOR(i,0,up.size()-1) C.pb(up[i]);
	FORD(j,down.size()-2,1) C.pb(down[j]);
	if (C.size() <= 2) return 0;
	double k;
	double total = 0;
	FOR(i,1,C.size()-2)
	{
		k = C[0].x * C[i].y + C[i].x * C[i+1].y + C[i+1].x * C[0].y - C[0].y * C[i].x - C[i].y * C[i+1].x - C[i+1].y * C[0].x;
		k = abs(k);
		k /= 2;
		total += k;
	}
...
int main()
{
	...
	Begin()
	{
		...
		cout<<setprecision(3)<<fixed<<getArea()<<endl;
	}
}
