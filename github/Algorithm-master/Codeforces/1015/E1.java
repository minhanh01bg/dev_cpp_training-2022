 ...
 * http://codeforces.com/problemset/problem/1015/E1
 */
...
public class E1 {
	...
	private static void grid(int y, int x) {
		int l = 0;
		for (int i = 1; x - i >= 0; i++) {
			if (map[y][x - i] == '*')
				l++;
		...
		if (l >= 1 && r >= 1 && t >= 1 && b >= 1) {
			int size = Math.min(l, Math.min(r, Math.min(t, b)));
			stars.add(new Star(y + 1, x + 1, size));
		}
	}
