...
long long A[15], per[15], ope[15],  B[15],  S, n;
...
{
	stack<long long> stc;
	stack<long long> st_char;
	stc.push(A[1]);
	for(int i=1; i<=n-1; i++)
	{
		if(ope[i] == 3)
		{
			long long tmp = stc.top(); stc.pop();
			stc.push((long long)tmp * A[i + 1]);
		}
		else{
			st_char.push(ope[i]);
			stc.push(A[i + 1]);
	...
	stack<long long> stcc, stc_char;
	while(!stc.empty())
	{
		stcc.push(stc.top()); stc.pop();
	}
	while(!st_char.empty())
	{
		stc_char.push(st_char.top()); st_char.pop();
	}
 
	while(stcc.size() > 1){
		long long val1 = stcc.top(); stcc.pop();
		long long val2 = stcc.top(); stcc.pop();
		long long o = stc_char.top(); stc_char.pop();
 
		if(o == 1) stcc.push(val1 + val2);
		else stcc.push(val1 - val2);
	}
	return stcc.top();
...
void backTrack(long long index)
{
	for(int i=1; i<=3; i++)
	{
		ope[index] = i;
