...
matrix<ll> convert(string s)
{
	ll m = s.length();
	matrix<ll> dfa(26, vec<ll>(m));
	dfa[s[0] - 'a'][0] = 1;
	for (ll X = 0, j = 1; j < m; j++) 
	{
		for (ll c = 0; c < 26; c++)
			dfa[c][j] = dfa[c][X];
			dfa[s[j] - 'a'][j] = j + 1;
...
matrix<ll> convert2(matrix<ll> dfa)
{
	ll x = dfa[0].size();
	matrix<ll> base(x + 1, vec<ll> (x + 1));
	for (int k = 0; k < 26; k++)
...
matrix<ll> matrix_mul(matrix<ll> a, matrix<ll> b, ll m)
{
	ll sz = a.size();
	matrix<ll> c(sz, vec<ll> (sz));
	FOR(i, 0, sz - 1)
	{
		FOR(j, 0, sz - 1)
		{
...
matrix<ll> power_matrix(matrix<ll> base, ll n, ll m)
{
	ll sz = base.size();
	matrix<ll> res(sz, vec<ll> (sz, 0));
	FOR(i, 0, sz - 1) res[i][i] = 1;
	while (n > 0) 
	{
		if (n % 2 == 1)
			res = matrix_mul(res, base, m);
...
void Process() 
{
	...
	string s;
	cin >> s;
	matrix<ll> dfa = convert(s);
	matrix<ll> base = convert2(dfa);
	matrix<ll> ans = power_matrix(base, a, m);
	cout << ans[0][s.length()] << '\n';
//	cout << ans << '\n';
}
