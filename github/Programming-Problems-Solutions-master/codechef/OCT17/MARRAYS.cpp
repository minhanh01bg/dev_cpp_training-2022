...
//    ios_base::sync_with_stdio(0);
    ...
    while(ti--)
    {
        ...
        for(int i = 0; i < n; i++)
        {
            ll pre1 = -mod*mod, pre2 = -mod*mod;
            //puts("----st-----");
            for(int j = 0; j < m; j++)
            {
                //printf("(%lld,%lld) ", dp[j], arr[(j+m-1)%m]);
                pre1 = max(pre1,dp[j]+arr[(j+m-1)%m]*i);
                pre2 = max(pre2,dp[j]-arr[(j+m-1)%m]*i);
            }
            //puts("\n----------");
            //deb("pre", pre1,pre2);
            scanf("%d", &m);
            ...
            {
                scanf("%lld", &arr[j]);
                dp[j] = max(pre1-arr[j]*i, pre2+arr[j]*i);
                //printf("%lld ", dp[j]);
            }
            //puts(">>>");
        }
