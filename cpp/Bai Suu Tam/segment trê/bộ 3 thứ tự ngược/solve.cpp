...
void Process()
{
	...
	getVector(C);
	map<ll, ll> M, M2;
	tree<p<ll, ll>, null_type,less<p<ll, ll> >, rb_tree_tag,tree_order_statistics_node_update> OS1, OS2;
	FOR(i, 0, a - 1)
	{
		M[C[i]]++;
		OS1.insert({-C[i], M[C[i]]});
	}
	ll total = 0;
	FORD(i, a - 1, 0)
	{
		ll k = OS1.order_of_key({-C[i], 0});
		ll q = OS2.order_of_key({C[i], 0});
		total += k * q;
		auto it = OS1.lower_bound({-C[i], M[C[i]]});
		OS1.erase(it);
		M[C[i]]--;
		M2[C[i]]++;
		OS2.insert({C[i], M2[C[i]]});
	}
	cout << total << endl;
}
