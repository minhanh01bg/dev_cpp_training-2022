...
//    freopen("input.txt", "r", stdin);
    ...
    for(int i = 1; i <= n; i++)
    {
        cin>>tem;
        mai[tem]++;
        int u = getid(tem.substr(0,k-1));
        int v = getid(tem.substr(1,k-1));
        //deb(u,v);
        adj[u].push_back(v);
        indeg[v]++;
        outdeg[u]++;
    }
    int st = 1, cnt1 = 0, cnt2 = 0;
    for(int i = 0; i <= mk.size(); i++)
    {
        //deb(i,indeg[i], outdeg[i]);
        if(abs(indeg[i]-outdeg[i]) >= 2) cnt1 = 100;
        if(indeg[i]<outdeg[i]) st = i, cnt1++;
        if(indeg[i]>outdeg[i]) cnt2++;
    }
    if(cnt1>=2 || cnt2>=2)
    {
        puts("-1");
    ...
    {
        //deb(">>>", ans.substr(i,k), i, i+k);
        if(mai.find(ans.substr(i,k)) != mai.end())
            mai[ans.substr(i,k)] --;
    }
