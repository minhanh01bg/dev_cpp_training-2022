...
int dijkstra(int start, int finish, int cafuel)
{
    ...
    while(!que.empty())
    {
        ...
        for(int i = 0; i < sz; i++)
        {
            ...
            if(fuel < ne_cost) continue;
            if(arr[ne_node][fuel-ne_cost] <= cost) continue;
            arr[ne_node][fuel-ne_cost] = cost;
            que.push(data(ne_node,cost,fuel-ne_cost));
        }
        if(arr[node][fuel+1] > cost+oil[node] && fuel < cafuel)
        {
            arr[node][fuel+1] = cost+oil[node];
            que.push(data(node,cost+oil[node],fuel+1));
        }
    }
    return -1;
}
