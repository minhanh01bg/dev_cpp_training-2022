...
ll cost(ll x)
{
    ...
    ll ans = mod*mod;
    pos = min(pos,(int) res.size()-1);
    for(int i = 0; i < 3 && ((i+pos)<res.size()); i++)
    {
        ans = min(ans, abs(x-res[i+pos]));
    }
    for(int i = 0; i < 3 && ((i-pos)>=0); i++)
    {
        ans = min(ans, abs(x-res[i-pos]));
    }
    return ans;
...
//    freopen("input.txt", "r", stdin);
    ...
    for(int i = n; i <= m; i++)
    {
        ...
        res.push_back(-even+odd);
        swap(even,odd);
    }
    ll total = 0;
    for(int i = 1; i <= n; i++)
    {
        if(i%2 == 1) total += arr1[i];
        else total -= arr1[i];
    }
    //deb(total);
