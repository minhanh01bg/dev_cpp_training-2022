...
void Try(ll i, ll multiply, ll count)
{
	if (i == 21) 
	{
		if (multiply % c == b)
	...
	ll multiply1 = (multiply * C[i]) % c;
	ll count1 = count + pow(2,i);
	Try(i+1, multiply, count);
	Try(i+1, multiply1, count1);
}
