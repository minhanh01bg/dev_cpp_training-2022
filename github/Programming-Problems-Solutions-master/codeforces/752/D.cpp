...
//    freopen("input.txt", "r", stdin);
    ...
    for(int i = 0; i <= mp.size(); i++)
    {
        ...
        reverse(arr[i][0].begin(), arr[i][0].end());
    ...
    for(int i = 0; i <= mp.size(); i++)
    {
        ...
        for(int j = 0; j < min(arr[i][0].size(), arr[i][1].size()); j++)
        {
            int tot = arr[i][0][j]+arr[i][1][j];
            if(tot>0)
            {
                ans += tot;
            }
    ...
    for(int i = 0; i <= mp.size(); i++)
    {
        ...
        cnt[i] = 0;
        for(int j = 0; j < arr[i][0].size(); j+=2)
        {
            if(j+1<arr[i][0].size())
            {
                if(arr[i][0][j+1]<0)
                {
                    extra[i][0] = arr[i][0][j];
                    extra[i][1] = arr[i][0][j+1];
                    cnt[i] = 2;
                    break;
                }else{
                    ans += arr[i][0][j]+arr[i][0][j+1];
                }
            }else{
                cnt[i] = 1;
                extra[i][0] = arr[i][0][j];
    ...
    for(int i = 0; i <= mp.size(); i++)
    {
        if(cnt[i] == 1) one = max(one, extra[i][0]);
    }
    ll tem = 0;
    for(int i = 0; i <= mp.size(); i++)
    {
        if(cnt[i] == 2)
        {
            if(extra[i][0]+extra[i][1]>0) tem += extra[i][0]+extra[i][1];
        }
    }
    ll mx = tem+one;
    for(int i = 0; i <= mp.size(); i++)
    {
        if(cnt[i] == 2)
        {
            if(extra[i][0]+extra[i][1]>0) mx = max(mx, tem-extra[i][1]);
            else mx = max(mx, tem+extra[i][0]);
        }
    }
    printf("%lld\n", ans+mx);
    return 0;
