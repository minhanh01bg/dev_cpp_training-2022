...
public class A {
	...
	private static void solve() {
		...
		while (T-- > 0) {
			...
			int l = ((x - 1) % d == 0) ? (x - 1) / d : (x - 1) / d + 1;
			int r = ((n - x) % d == 0) ? (n - x) / d : (n - x) / d + 1;
			...
			if ((n - y) % d == 0)
				second = r + ((n - y) / d);
			if (Math.abs(y - x) % d == 0)
				third = Math.abs(y - x) / d;

			int answer = Math.min(first, Math.min(second, third));
			io.println((answer == Integer.MAX_VALUE) ? -1 : answer);
		}
	}
