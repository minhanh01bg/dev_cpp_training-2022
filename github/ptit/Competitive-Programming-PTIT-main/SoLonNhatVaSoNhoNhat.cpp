...
{
	return factor[n] * inverse(factor[k] * factor[n-k] % M) % M;
}

int main()
{
//	fstream f("10in(2).txt", ios::in);
	faster();
	factory();
//	Begin()
//	{
...
//	}
	...
	FORD(i,n-k,0)
	{
		F[i] = (F[i+1] + binomialCoeff(n-2-i,k-2) % M) % M;
//		cout<<F[i]<<endl;
	}
	FOR(i,0,n-1)
	{
		total = (M + total + (F[n-i-1] % M) * (C[i] * 1LL))%M;
		total = (M + total - (F[i] % M) * (C[i] * 1LL))%M;
	}
	cout<<total%M<<endl;
//	f.close();
}
