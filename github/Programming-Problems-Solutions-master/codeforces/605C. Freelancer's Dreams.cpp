...
bool pointinpoly(int node, Point *arr, double xx, double yy)  //nodes should be given in clockwise or anti-clockwise order
{
  int i, j;
  bool c=false;
  for (i = 0, j = node; i <= node; j = i++) {
...
int main() {
    ...
    arr[n++] = Point(mx,0);
    arr[n++] = Point(0,my);
    int len = graham(arr,n,s);
    //csprnt;
    //print1(top);
    double low = 0, hi = 4000000.0, mid;
    for(int i = 0; i < 400; i++){
        mid = (low+hi)/2;
        if(pointinpoly(len-1,s,(double) a/mid,(double) b/mid)){
            hi = mid;
        }else low = mid;
    }
    printf("%.10lf\n", mid);
