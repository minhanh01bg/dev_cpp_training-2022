...
ll getval(int l, int r)
{
    for(int i = l; i <= r; i++)
        printf("%d ", i);
    if(l == 0) return lev[r];
    return lev[r]^lev[l-1];
}

ll solve(int pos, int n, int l, int r)
{
    //deb(pos,n,l,r);
    if(pos == 0) return getval(l,l);
    if(pos == 1) return getval(l,r);
    int m = (l+r+1)/2;
    if(pos<=(n/2))
    {
        return solve(pos,n/2,l,m-1)+solve(pos,n/2,m,r);
    }
    return solve(pos-(n/2),n/2,l,m-1);
}
