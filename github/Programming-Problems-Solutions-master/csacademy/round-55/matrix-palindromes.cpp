...
int both(char a, char b, char c, char d) {
...
//    freopen("00.txt", "r", stdin);
    ...
    for(int i = 0; i*2 < n; i++)
        for(int j = 0; j*2 < m; j++) {
            if(i != (n-i-1) && j != (m-j-1))
                col[j] += both(arr[i][j], arr[i][m-j-1], arr[n-i-1][j], arr[n-i-1][m-j-1]);
            else
                col[j] += both(arr[i][j], arr[i][m-j-1], arr[n-i-1][j], arr[n-i-1][m-j-1])/2;
            row[j] += (arr[i][j] != arr[i][m-j-1])+((i != (n-i-1))&&(arr[n-i-1][j] != arr[n-i-1][m-j-1]));
        }
    int ans = 0;
    for(int i = 0; i < p; i++) {
        ans += row[i];
    ...
    for(int i = 0; i*2<k; i++)
        tem += col[i];
    if(m%2 == 1 && k%2 == 1)
        tem = min(tem,tem-col[k/2]+col[p-1]);
    printf("%d\n", ans+tem);
    return 0;
}
