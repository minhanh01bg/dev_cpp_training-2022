...
ll check(int i, vec(ll) Hash)
{
	...
	FOR(j,0,i-1)
	{
		ll op = Hash[j] + Hash[k-1-j];
		maximum = max(maximum,op);
	}
	maximum = max(maximum,Hash.back());
	total = maximum * i + maximum * f;
	return total;
}
...
//	}
	...
	getVector(C);
	ll minimum = INT_MAX;
	FOR(i,0,a-1) Hash[C[i]-1]++;
	SORT(Hash);
	ll k = b/2;
	if (b&1) k++;
	FOR(i,k,b)
	{
		minimum = min(minimum, check(i,Hash));
	}
	cout<<minimum<<endl;
}
